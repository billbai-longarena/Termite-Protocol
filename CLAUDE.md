...
11. Audit package lacks result verification (F-010, status: open) — current design captures process metrics (commits, observations, handoffs) but no outcome quality data. Cannot verify "174 tests passing" or "Production-ready ECS" claims. Proposed: add `result-snapshot.yaml` with build status, test results, demo output, code metrics.
