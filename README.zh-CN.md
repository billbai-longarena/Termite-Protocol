...
| **白蚁 .birth (v3+)** | **~2%** 上下文窗口 | **800 token 动态计算的快照** |
| CrewAI 角色 prompt | ~10-15% | 角色描述 + 对话历史 |
| AutoGen 系统 prompt | ~5-10% | 系统消息 + 增长的聊天记录 |

`field-arrive.sh`（434 行）动态计算 `.birth`，包含：蚁丘当前状态、最高优先级未认领信号、行为模板（牧羊效应示范）、4 条安全规则、恢复提示。Agent 需要知道的一切，不多不少。
...
```
v2 之前:  Agent → 读 TERMITE_PROTOCOL.md (28K tokens) → 上下文消耗 40%
v3 之后:  Agent → field-arrive.sh 计算 .birth → Agent 读 .birth → 上下文消耗 2%
```
...
| **touchcli** (A-005) | Codex + 2 Haiku | 6h | 130 | 6 | **牧羊效应验证**：96.4% 质量 |
...
| 指标 | A-003（纯弱模型） | A-005（牧羊效应） | A-006（5 模型混合） |
| --- | --- | --- | --- |
| 观察退化率 | 64% | 3.6% | 57% |
| 交接质量 | 0% | 99% | 100% |
| 规则涌现 | 0 | 1 | 21（全部退化） |
...
升级：`bash /tmp/termite/install.sh --upgrade`
...
| 部分 | 内容 | 面向 |
| --- | --- | --- |
| **I: 语法** | 9 条规则 + 4 条安全网 | field-arrive.sh 提取 → .birth |
| **II: 环境配置** | 种姓、信号 schema、降级矩阵 | field-arrive.sh 读取 |
| **III: 人类参考** | 完整设计理由、免疫系统 | 人类阅读 |
| **IV: 附录** | 模板、快速参考卡 | 按需查阅 |
...
| `termite-db.sh` | 数据库：SQLite WAL-mode + 迁移 |
...
| **v3.5** | 数据库架构（SQLite WAL）、牧羊效应、蚁丘生命周期检测 |
